...
class TalosPickPlace(TalosGl):
	...
	def pick(self, obj_to_grasp, id_group): 
		...
		obj_pose_wrist[1] = obj_pose_wrist[1] - self.const.APPROACH_PICK_Y[id_group] - self.const.MARGIN_Y[id_group] - self.const.DIST - dim_y/2
	...
	def place(self, obj_to_grasp, last_pose, id_group):
		...
		place_approach_pose_wrist[1] = place_approach_pose_wrist[1] + self.const.APPROACH_PLACE_Y[id_group] - self.const.DIST -  dim_y/2 + self.const.MARGIN_Y[id_group]
	...
	def move_switch_point(self, obj_to_grasp):
		...
		p.pose.position.y = self.const.SWITCH_POSITION.get("y") - self.const.DIST - self.const.MARGIN_Y[0] - dim_y/2
		...
		p.pose.position.y = p.pose.position.y + 2 * (self.const.DIST + self.const.MARGIN_Y[1]) + dim_y + self.const.APPROACH_PICK_Y[1]
